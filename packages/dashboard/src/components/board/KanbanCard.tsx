...
export function KanbanCard({ card, onPlay, onStop, onClick, onCoPlan, isDragOverlay, isDraggable }: KanbanCardProps) {
  const isRunning = card.status === "in-progress";
      ...
      <CardContent className="p-3">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-start gap-1.5 flex-1 min-w-0">
            ...
            <h4 className={cn("font-medium text-sm leading-tight break-words [overflow-wrap:anywhere]", !card.title && "text-muted-foreground")}>{cardLabel(card)}</h4>
          </div>
          {!isRunning && onCoPlan && (
            <Button
              variant="ghost"
        ...
        {card.title && card.description && (
          <p className="text-xs text-muted-foreground mt-1 line-clamp-2 ml-5 break-words [overflow-wrap:anywhere]">
            {card.description}
          </p>
        )}
